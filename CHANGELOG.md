...
## 3.10.3 - Unified Creator condition selector everywhere

- Retires the deprecated stacked three-level selector on ALL known condition surfaces: Rule / Label Builder, Secondary Metrics Creator Library results, and the main Creator Library.
- Every condition is rendered as one row: JOIN -> Level 1 -> Level 2 -> Level 3 -> operator -> value -> delete. Standalone Search buttons are hidden and Level-3 search is embedded in the selector.
- Carries forward V3.10.2 Saved Metrics / Rule List behavior: 10 items per page and internal vertical scrolling with paired panel heights.
- Existing SQLite data, saved metrics, rules, filters and execution semantics are unchanged.

## 3.10.2 - Secondary Metrics interaction consolidation

- Saved Metrics and Rule List now use the native paginator only; the V3.10.1 duplicate pager is removed. Page size is normalized to 10 and each list gets an internal vertical scrollbar while preserving paired panel heights.
- Rule / Label Builder now uses the same compact one-row three-level condition selector as Creator Library result filters. The deprecated stacked hierarchy and standalone Search button are no longer shown.
- Third-level metric search remains embedded in the selector; rule/filter execution semantics and existing SQLite data are unchanged.

## 3.10.1 - Secondary Metrics UI refinement

- Secondary Metrics workspace: right-side Saved Metrics / Rule List panels now align with their left-side builders and paginate internally instead of stretching the page.
- Creator Library result filters: three-level metric hierarchy is compacted into one row per condition; legacy redundant selector is removed from the visible flow and third-level metric search is embedded into the selector instead of occupying a separate row.
- Responsive fallback keeps the filter usable on narrower windows without changing metric/rule execution semantics.

# Changelog

## v3.10.0
- Field Registry 升级为三级目录：客观数据 / 博主标签 / 构建指标 / 比值指标 → 二级业务维度/用户分组 → 具体字段；筛选、排序、规则、指标构建和 Saved View 使用稳定 Field ID。
- JobStore 升级为持久 Job Engine：资源队列并发限制、取消、重试、checkpoint、可恢复批任务和重启恢复；任务中心显示队列/资源状态。
- Schema 16 → 17，新增正式 `schema_migrations`、`run_specs`、`data_assertions`；旧 Schema 16 自动登记 baseline 并执行 migration。
- 新增 `/api/v1` 稳定接口层和统一 success/error envelope；legacy API 保持兼容。
- AI Search / Ask Hub 生成不可变 Run Specification；Clone & Re-run 对 AI Search 复用冻结 Plan、实际 Query 与 Fit Criteria，不重新规划。
- 建立 Fact / Derived / AI / Human / Effective 数据断言契约；视频复核、频道人工覆盖、AI objective fit 已发布 assertion。
- Weekly Intelligence Engine V3：AI 候选按 Creator 去重、Workflow 变化读取 audit、GMV 计算最新快照 delta、识别卡住同步，并过滤近期已知风险/官方来源的 Top Discovery。
- Service 拆分第一阶段：RunService、IntelligenceService、DataContractService 独立，CreatorHub 保留兼容 facade。
- 发布自检增加 Job Engine、三级 taxonomy、migration、data contract、run spec 等架构断言。

## v3.9.5
- Added shared Field Registry metadata for creator facts, labels, video facts and user-defined metrics.
- Added searchable grouped Field Picker with favorites and recent fields.
- Secondary-metric sorting, filters, rule conditions, ratio operands and video source selection reuse the shared field catalog.
- Creator-library sorting/filtering uses the same field grouping metadata; Saved Views retain stable field IDs.
- Schema remains 16.

## v3.9.4

- 【已构建指标】与【规则列表】升级为配置目录：支持搜索、分组筛选、未分组、排序、30 条分页，以及批量移动到分组；输入新分组名即可创建分组。
- 【监控健康】新增搜索、频道状态/同步健康/监控状态/优先级结构化筛选，以及处理优先级、博主、频道状态、同步健康、优先级、最近同步、数据龄、失败次数、下次同步/重试等排序。
- 【监控健康】主表执行“结论优先”：频道状态仅显示有效状态 + 人工确认/系统检测；系统原始状态、判断原因、内容状态、监控策略、连续不可用次数、错误信息等进入右侧状态详情。
- 选择列统一保证完整显示；发布视觉回归新增二次指标页并检查监控健康选择表头、配置目录分页与页面水平溢出。
- Schema 保持 16；GMV 继续固定为 USD 累计快照。

## v3.9.3

- GMV 业务口径固定为 UgPhone 后台 **USD 累计快照**；保留采集时间，不跨快照求和。
- 升级时自动把历史 `gmv` 行回填为 USD/native，修复 V3.9.1–V3.9.2 产生的大量“待换算”状态；无需重新导入原始商业数据。
- 删除商业表现页 GMV 汇率状态、币种补全、在线 FX 解析入口；服务端对应 FX Job/API 停用。
- 新导入 GMV 始终按 USD 保存，即使旧工作簿存在币种/汇率列也不再执行换算。
- V3.9.2 的任务中心固定尺寸、AI 双维度适配、Query 漏斗和 Weekly Brief V2 保持不变。
- Schema 保持 16；旧 FX 字段仅为数据库向后兼容保留，不再参与 GMV 业务计算。

## v3.9.1

- 商业指标改为采集时间点累计快照语义；不同 `captured_at` 的累计 GMV 不再相加，主界面与二次指标默认使用最新快照。
- `creator_business_metrics` 新增 USD 标准值及 FX 元数据：`metric_value_usd`、`fx_rate_to_usd`、`fx_rate_date`、`fx_provider`、`fx_status`、`snapshot_kind`；Schema 15 → 16。
- 商业导入支持采集时间列和 Dashboard 手工采集时间；CLI `import-business` 新增 `--capture-at`。
- 非 USD 金额优先使用源表兑美元汇率，否则按快照日期解析公开汇率；无法换算时明确标记“待换算”，不参与 USD headline。
- AI Search 增加 Creator 内容语言 Profile，默认目标语言有效样本占比 ≥60%；样本不足则“未验证”，而不是误杀。
- 扩充多开/多账号场景词到 alt/alts/multiple alts/multi client/multiple clients/farm with alts 等社区表达。
- AI Result Set 增加推荐/风险候选池、Creator 语言字段和代表性适配视频；XLSX 增加 Risk Candidates Sheet。
- Planner Strategy 改为从最终 Fit Criteria 确定性生成；Query Planner Prompt 升至 v7。
- 视频分类默认列预算进一步收敛，系统/分类来源只在筛选或审计时按需显列。
- 新增真实 Chromium 视觉回归脚本，覆盖 1366×768、1600×900、1920×1080 和 125% 缩放。
