# YouTube 博主数据中心 v3.10.0
> v3.10.0 是 V3 Core Architecture 版本：三级 Field Taxonomy、持久 Job Engine、正式 Migration Runner、`/api/v1`、冻结 Run Specification、统一数据断言契约与 Creator Intelligence Engine V3。Schema 16 → 17。

## v3.10.0：Core Architecture
- **三级 Field Taxonomy**：一级固定为「客观数据 / 博主标签 / 构建指标 / 比值指标」；二级为业务维度或用户自定义指标组；三级才是具体字段。筛选、排序、规则、比值构建和 Saved View 共用稳定 Field ID。
- **Job Engine v1**：后台任务进入持久队列，按 YouTube / AI / Local / Maintenance 资源池限制并发；支持 cooperative cancel、失败重试、可恢复批任务 checkpoint，以及 Dashboard 重启后的安全续跑/中断判定。
- **Migration Runner**：新增 `schema_migrations`，迁移有版本、名称和 checksum。V3.9.x Schema 16 会记录 legacy baseline 后执行 16→17 迁移，不再只依赖启动时散落的兼容 UPDATE。
- **Stable API v1**：新增 `/api/v1/...` 数据契约与统一 success/error envelope；旧 `/api/...` 继续保留给当前 Dashboard，便于后续 Workbench 前端逐步迁移。
- **Run Specification**：AI Search/Ask Hub 将请求、最终 Plan、实际 Query、Profile Budget、模型和 Prompt 版本冻结到 `run_specs`；Clone & Re-run 复用冻结 Query / Fit Criteria，不重新让 Planner 改写实验条件。
- **Fact / Derived / AI / Human / Effective**：新增 `data_assertions` 与统一优先级契约；视频人工分类、频道人工生命周期、AI objective fit 已接入，旧业务表继续兼容。
- **Creator Intelligence Engine V3**：Weekly Context 按 Channel ID 去重近期 AI 候选，读取真实 workflow audit 变化、GMV 最新快照差值、卡住同步任务和风险过滤后的 Top Discovery；核心 KPI 仍由确定性 SQL 生成。
- **Service 拆分第一阶段**：Run、Intelligence、Data Contract 已从 `CreatorHub` God Object 中拆成独立 service；V3.10 保留 facade，避免破坏既有 CLI/Dashboard。
- Schema 升至 **17**；安装包不包含数据库，`upgrade.cmd` 会先备份再执行 migration/self-check。

## v3.9.5：Field Registry 基础
- 建立共享 Field Registry 元数据与稳定 field ID。
- 引入可搜索/分组的 Field Picker、最近使用和收藏；二次指标、规则、比值构建与博主库开始复用字段目录。
- 自定义指标组进入字段元数据，Saved Views 继续保存稳定 field ID。
- 该版仍为单层大 Picker；V3.10.0 已升级为三级 Field Taxonomy。Schema 保持 16。

> v3.9.4 统一二次指标配置目录与监控健康表格能力：分组/搜索/排序/分页完整化，监控健康主表改为结论优先；Schema 保持 16，GMV 仍固定为 USD 累计快照。
> v3.9.2 的任务中心、AI 双维度适配、Query 漏斗和 Weekly Brief V2 全部保留。
...
- **已构建指标 / 规则列表**：均支持搜索、分组筛选、排序、每页 30 条分页；可多选后批量移动到已有或新建分组，也可批量设为“未分组”。
- **监控健康**：支持博主搜索、频道状态/同步健康/监控状态/优先级筛选，并支持处理优先级、最近同步、数据龄、失败次数、下次同步/重试等排序。
- **结论优先**：监控主表不再堆叠系统原始检测、人工备注、内容状态、监控策略、连续不可用次数和错误信息；这些信息统一进入“状态详情” Inspector。
- **发布检查**：真实 Chromium 视觉回归增加 `metrics.html`，同时检查监控健康选择表头完整性及宽度溢出。
- **兼容性**：Schema 仍为 16；V3.9.3 的 GMV USD 快照口径保持不变。

## v3.9.3：GMV 固定 USD 口径

- UgPhone 后台导入的 `GMV` 默认且固定解释为 **USD 累计 GMV**，每次导入按 `captured_at` 保存时间点快照。
- 同一 Creator 不同采集时间的累计 GMV 不相加；博主库、二次指标、AI/Brief 默认读取最新采集时间点。
- 升级时自动把既有所有 `gmv` 历史行回填为 `currency=USD`、`metric_value_usd=metric_value`，因此旧数据无需重新导入，也不会继续显示“待换算”。
- 删除 Dashboard 的汇率状态、币种补全、在线 FX 查询和 GMV 换算入口；新导入 GMV 即使旧表含币种/汇率列，也按后台 USD 口径保存。
- 任务中心、AI Search 双维度适配、Query 漏斗、Weekly Brief V2 与视觉回归保持 v3.9.2 行为。

## v3.9.1：商业快照、USD 标准化与 AI sourcing 精修

- 后台导入的 GMV/Revenue/Commission/Cost 视为**采集时间点的累计快照**。同一 Creator 的历史快照永久保留，但博主库/二次指标默认只使用该指标**最新采集时间**的快照，禁止把不同时间点累计值相加。
- `captured_at` 优先读取表格中的“采集时间/数据时间/截至时间”等列；没有时可在 Dashboard 导入前填写采集时间；仍未提供时才使用导入时间。
- **历史说明（已被 v3.9.3 覆盖）**：v3.9.1 曾加入通用 FX 标准化；从 v3.9.3 起 GMV 明确按 UgPhone 后台 USD 口径处理，不再执行 GMV 换算。
- AI Search 的 Language 同时作为搜索语言和 Creator 主要内容语言目标；最近标题有效语言样本充足时，目标语言占比默认至少 60%。
- 多开/多账号扩展到 `alt/alt account/alts/multiple alts/multi client/multiple clients/farm with alts` 等玩家表达。
- AI Result Set 显式区分“推荐候选 / 风险候选 · 需人工复核”，并保存“代表性适配视频”与“最佳搜索命中视频”两个不同证据。XLSX 额外输出 Risk Candidates Sheet。
- Planner Strategy 改为从最终结构化 Fit Criteria 确定性生成，避免解释文案与实际硬约束冲突。Query Planner Prompt 升至 `query-planner-v7`。
- 新增 `scripts/visual_regression.py`：发布时用真实 Chromium 在 1366×768、1600×900、1920×1080 与 125% 缩放场景检查主页面无横向溢出/单元格越界。
...
覆盖旧版本后，先双击 `upgrade.cmd`。v3.9.1 会先使用 SQLite Backup API 对现有 `data/creator_hub.sqlite` 创建 `backups/pre_upgrade_*.sqlite` 一致性备份，再执行 Schema 迁移、自检、清理旧 Dashboard 并重新生成。
