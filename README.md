# YouTube 博主数据中心 v3.10.5

> 本仓库是 **YouTube Creator Data Hub v3.10.5** 的完整源码版本。  
...
V3.10.5 对规则编辑区做了进一步的纵向结构优化，避免单条规则时 Card 过度收缩：

- 桌面端 `规则 / 标签构建器` 最小高度约 **500px**；
- 内部拆分为三个区域：
  1. 规则名称 / 分组 / 说明；
  2. 条件设置；
  3. 保存 Footer；
...
- `添加条件` 独立放在条件区域底部；
- `保存规则 / 清空` 放在独立 Footer；
- `规则列表`继续跟随左侧 Builder 高度；
- 小屏幕下取消固定最小高度，恢复自然纵向布局。
