# YouTube 博主数据中心 v3.10.6

> 本仓库是 **YouTube Creator Data Hub v3.10.6** 的完整源码版本。  
...
### V3.10.6 规则 / 标签构建器固定工作区

V3.10.6 将规则区改为**固定外壳 + 内部滚动**，条件数量不再参与外层 Card 高度计算：

- 桌面端 `规则 / 标签构建器` 使用约 **690px 固定外壳高度**；
- 条件工作区固定为可完整查看约 **3 条条件**的 viewport；
- 第 4 条及以后条件只在条件工作区内部通过**垂直滚动条**查看；
- 新增或删除条件不会改变规则构建器外壳高度；
- 规则名称 / 分组 / 说明固定在顶部；
- `添加条件`、`保存规则 / 清空`和说明区域固定在下方；
- `规则列表`使用完全相同的固定外壳高度；
- 规则列表只有中间 `ruleList` Body 可以垂直滚动；
- 单个规则卡片保持自然高度，禁止 flex-grow / stretch，因此不会出现规则卡片下方被拉出大块空白；
- 规则列表继续固定 **10 条 / 页**；
- 小屏幕下取消桌面固定外壳，恢复自然纵向布局。
...
固定 Rule Builder 外壳
├─ 规则元数据：固定
├─ Conditions viewport：固定高度 + overflow-y:auto
└─ 操作 Footer：固定

固定 Rule List 外壳
├─ 搜索 / 批量工具：固定
├─ ruleList：flex:1 + overflow-y:auto
└─ Pager：固定
```

这项调整只改变 Dashboard 布局，不改变规则结构、执行语义或 SQLite 数据。
