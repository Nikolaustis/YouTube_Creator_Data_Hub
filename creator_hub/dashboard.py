...
def build_dashboard(db_path: str | Path, output_dir: str | Path, settings: dict[str, Any]) -> dict[str, Any]:
    ...
    with connect(db_path) as conn:
        ...
        for c in creators:
            c['monitor_data_fresh']=1 if monitoring_data_fresh(
                settings, priority=c.get('priority'), last_synced_at=c.get('last_synced_at')
            ) else 0
            c['suspected_inactive_partner']=1 if suspected_inactive_partner(
                settings, monitoring_enabled=c.get('monitoring_enabled'), priority=c.get('priority'),
                last_synced_at=c.get('last_synced_at'), ugphone_video_count=c.get('identified_ugphone'),
                latest_ugphone_upload=c.get('latest_ugphone_upload'), inactive_days=30
            ) else 0
        ...
        <div class="section"><div class="note"><b>身份口径：</b>存在 UgPhone 视频 → “合作过博主”；不存在 → “未合作博主”。若检测到对应竞品视频，则分别标记为“LDCloud合作博主”“RedFinger合作博主”“VSPhone合作博主”。历史上存在 UgPhone 视频、仍在监控、最近一次同步数据仍新鲜且连续30天没有新的 UgPhone 视频时，额外标记橙色“疑似不再合作”；该标签是待核查状态，不覆盖“合作过博主”的历史事实。人工修正仅覆盖系统误判。<br><b>监控口径：</b>“监控中”表示该博主进入批量同步队列；“高 / 普通 / 低 / 归档优先级”分别按配置周期判断是否到期，未到期会在批量同步时跳过。</div></div>
        <div class="section"><h2>博主库</h2><div class="toolbar"><input id="q" class="input" placeholder="搜索博主 / Handle / Channel ID / 国家 / 身份标签"></div><div class="builder-panel"><div id="ovFilterConditions"></div><div class="inline"><button class="btn" id="ovAddFilter">添加筛选条件</button><button class="btn primary" id="ovApplyFilter">应用筛选</button><button class="btn" id="ovClearFilter">清除</button><span class="small">博主标签为布尔标签，可直接选择“存在 / 不存在”，不填写数字阈值。</span><span id="ovFilterStatus" class="small"></span></div></div><div class="toolbar section"><span class="small">排序</span><select id="ovSort" class="select"><option value="ugphone_video_count" selected>UgPhone视频数</option><option value="channel_title">博主名称</option><option value="country">国家</option><option value="subscriber_count">订阅数</option><option value="channel_view_count">频道累计播放量</option><option value="stored_videos">已存视频数</option><option value="latest_upload">最近发布</option><option value="ldcloud_video_count">LDCloud视频数</option><option value="redfinger_video_count">RedFinger视频数</option><option value="vsphone_video_count">VSPhone视频数</option></select><select id="ovSortDir" class="select"><option value="desc" selected>降序</option><option value="asc">升序</option></select>{_page_size_controls("ov")}<span id="ovSummary" class="table-summary"></span></div><div class="table-wrap"><table><thead><tr><th>博主</th><th>国家 / 证据</th><th>订阅数</th><th>频道累计播放量</th><th>已存视频数</th><th>最近发布</th><th>身份标签</th><th>UgPhone视频数</th><th>竞品品牌视频数</th><th>数据状态</th></tr></thead><tbody id="rows">{''.join(rows)}</tbody></table></div>{_pager("ov")}</div>
        <script src="assets/creator_facts.js"></script><script src="assets/metric_base.js"></script><script src="assets/geography.js"></script><script src="assets/table_tools.js"></script><script src="assets/overview_filters.js"></script>'''
        ...
        (assets/'query_packs.js').write_text('window.CDH_QUERY_PACKS='+json.dumps(qp_obj,ensure_ascii=False,separators=(',',':'))+';\n',encoding='utf-8')
        ...
        <div class="toolbar"><input id="labelSearch" class="input" placeholder="搜索视频 / 博主 / Video ID"><select id="labelSort" class="select"><option value="published">发布时间</option><option value="views">播放量</option><option value="likes">点赞数</option><option value="comments">评论数</option><option value="duration">视频时长</option><option value="creator">博主</option><option value="title">视频标题</option><option value="role">最终分类</option><option value="review_status">复核状态</option></select><select id="labelSortDir" class="select"><option value="desc">降序</option><option value="asc">升序</option></select>{_page_size_controls('label')}<span id="labelPageInfoTop" class="small"></span><span id="labelSummary" class="table-summary"></span></div>
        <div class="builder-panel"><div id="labelFilterConditions"></div><div class="inline"><button class="btn" id="labelAddFilter">添加筛选条件</button><button class="btn primary" id="labelApplyFilter">应用筛选</button><button class="btn" id="labelClearFilter">清除筛选</button><span class="small">支持最终/系统分类、品牌、复核状态，以及播放量、点赞数、评论数、视频时长和发布时间等视频客观数据。</span><span id="labelFilterStatus" class="small"></span></div></div>
        ...
        <div class="builder-panel section"><h2>搜索新博主</h2>
          <div class="toolbar"><input id="discoverQuery" class="input" style="min-width:360px" placeholder="输入游戏名称，例如 Anime Expeditions"><select id="discoverSource" class="select"><option value="web">YouTube 网页搜索</option><option value="api">YouTube API 搜索</option></select><select id="discoverLookback" class="select"><option value="">不限发布时间</option><option value="7">近7天</option><option value="30">近30天</option><option value="60">近60天</option><option value="90">近90天</option><option value="180">近180天</option><option value="365">近365天</option><option value="custom">指定日期范围</option></select><span id="discoverCustomDates" class="inline" style="display:none"><input id="discoverFromDate" class="input" type="date" style="min-width:145px"><span class="small">至</span><input id="discoverToDate" class="input" type="date" style="min-width:145px"></span></div>
          <div class="toolbar"><select id="discoverRegionGroup" class="select"><option value="">全部大洲/区域</option></select><input id="discoverCountrySearch" class="input" list="countrySuggestions" placeholder="输入中文国家名或英文代码，例如 菲律宾 / PH" style="min-width:290px"><datalist id="countrySuggestions"></datalist><select id="discoverCountry" class="select"><option value="">不限国家/地区</option></select><span class="small">每个 Query 视频上限</span><select id="discoverMax" class="select"><option>25</option><option selected>50</option><option>100</option><option>200</option></select></div>
          <div class="builder-panel" style="box-shadow:none;margin-top:10px"><div class="inline spread"><div><b>Query Expansion</b><div class="small">原关键词始终搜索一次；启用 Query Pack 后，仅勾选的扩展词参与本次搜索。取消勾选只停用本次检索，× 会从当前语言词库删除。</div></div><div class="inline"><span class="small">长尾语言</span><select id="queryLanguage" class="select"></select><button class="btn" id="queryResetDefaults">恢复当前语言默认词</button></div></div><div id="queryPackGrid" class="qe-grid section"></div><div class="section"><div class="small">本次将执行的 Query</div><div id="queryPreview" class="qe-preview"></div><div id="queryCost" class="small" style="margin-top:6px"></div></div></div>
        ...
        <script src="assets/creator_facts.js"></script><script src="assets/geography.js"></script><script src="assets/query_packs.js"></script><script src="assets/table_tools.js"></script><script src="assets/discovery.js"></script>'''
