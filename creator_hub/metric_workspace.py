...
<div class="section metric-builder"><div>
  ...
  <div class="builder-panel section"><h2>规则 / 标签构建器</h2><input type="hidden" id="ruleEditId"><div class="form-row"><label>规则名称</label><input id="ruleName" placeholder="例如：高潜未合作博主"></div><div id="ruleConditions"></div><div class="inline"><button class="btn" id="addRuleCondition">添加条件</button><button class="btn primary" id="saveRule">保存规则</button><button class="btn" id="clearRule">清空</button></div><div class="small">条件类型包括博主客观数据、博主标签、构建指标和比值指标。博主标签使用“存在 / 不存在”，不填写数字阈值。第二条起可逐条选择 AND / OR / NOT。</div></div>
</div><div><div class="builder-panel"><div class="inline spread"><h2>已构建指标</h2><span class="small">这里只保存用户构建的构建指标和比值指标</span></div><div id="metricList" class="metric-list section"></div></div><div class="builder-panel section"><div class="inline spread"><h2>规则列表</h2><span class="small">规则作用对象始终是博主</span></div><div id="ruleList" class="metric-list section"></div></div></div></div>
<div class="section"><div class="title compact-title"><div><h1>应用结果 · 博主库</h1><div class="sub">全部条件最终都在博主粒度上执行。可添加多条筛选条件，并逐条使用 AND / OR / NOT。</div></div></div>
