...
function identityPills(c){const a=Object.entries(creatorLabels).filter(([k])=>Number(c[k]||0)).map(([,label])=>label);return a.length?a.map(t=>`<span class="pill identity-partnered">${esc(t)}</span>`).join(''):'<span class="small">—</span>'}
...
function fixedKindForMetric(m){if(!m||m.type!=='constructed'||m.source_kind!=='video_fact'||m.source_field!=='current_views'||(m.window||'all')!=='all'||(m.aggregation||'count')!=='median')return '';return (m.filter_label||'')?'':'all'}
