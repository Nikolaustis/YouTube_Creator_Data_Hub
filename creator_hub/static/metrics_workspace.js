...
function migrateState(x){
  ...
  return {...emptyState(),...x,schema_version:1,metrics,rules,filters};
}
function load(){
  try{
...
function metricComparable(m){
  if(m.type==='constructed')return {id:m.id,name:m.name,type:m.type,visible:!!m.visible,source_kind:m.source_kind,source_field:m.source_field,filter_label:m.filter_label,window:m.window,from_date:m.from_date||'',to_date:m.to_date||'',aggregation:m.aggregation};
  return {id:m.id,name:m.name,type:m.type,visible:!!m.visible,numerator_ref:m.numerator_ref||null,denominator_ref:m.denominator_ref||null};
}
...
function clearMetric(){document.getElementById('metricEditId').value='';document.getElementById('metricName').value='';document.getElementById('metricOutputType').value='constructed';document.getElementById('metricVisible').value='1';document.getElementById('metricStatus').textContent='';resetMetricDrafts();renderMetricDynamic()}
async function saveMetric(){
  ...
  try{
    ...
    const id=requireElement('metricEditId','编辑状态').value,name=requireElement('metricName','指标名称').value.trim(),type=requireElement('metricOutputType','输出类型').value,visible=requireElement('metricVisible','显示状态').value==='1';
    if(!name)throw new Error('请输入指标名称');if(!['constructed','ratio'].includes(type))throw new Error('输出类型无效');
    const old=metricById(id),m={id:id||uid('m'),name,type,visible,version:(old?.version||0)+1,updated_at:iso()};
    if(type==='constructed'){
    ...
    clearMetric();renderAll();st.textContent=`已按当前设置保存：${m.name}`;
...
function editMetric(id){
  ...
  document.getElementById('metricEditId').value=id;document.getElementById('metricName').value=m.name;document.getElementById('metricVisible').value=m.visible?'1':'0';document.getElementById('metricOutputType').value=m.type;renderMetricDynamic(m);
}
...
function saveRule(){const id=document.getElementById('ruleEditId').value,name=document.getElementById('ruleName').value.trim(),conditions=readConditions('rule');if(!name)return alert('请输入规则名称');if(!conditions.length)return alert('至少添加一个有效条件');const old=state.rules.find(x=>x.id===id),r={id:id||uid('r'),name,conditions,version:(old?.version||0)+1,updated_at:iso()};if(old)Object.assign(old,r);else state.rules.push(r);clearRule();persist()}
...
document.getElementById('addResultFilter').onclick=()=>addCondition({},'filter');document.getElementById('applyFilter').onclick=()=>{state.filters=readConditions('filter');persist()};document.getElementById('clearFilter').onclick=()=>{state.filters=[];persist()};
...
document.getElementById('resetCfg').onclick=()=>{if(confirm('清空全部已构建指标和规则？')){state=emptyState();localStorage.removeItem(KEY);renderAll()}};
