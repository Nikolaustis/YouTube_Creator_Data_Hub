...
fetch('/api/ping',{cache:'no-store'}).then(r=>r.ok?r.json():Promise.reject()).then(x=>{box.className='runtime-status runtime-live';box.innerHTML=`<b>运行模式：交互模式</b><span>Python ${x.python||'—'} · SQLite ${x.db_exists?'已连接':'未连接'} · API Key ${x.api_key_present?'已配置':'未配置'} · <span class="mono">.1:8765</span>（仅本机）</span>`;window.CDH_INTERACTIVE=true;window.dispatchEvent(new CustomEvent('cdh-interactive-ready',{detail:x}))}).catch(()=>{window.CDH_INTERACTIVE=false;staticMode()});
})();
