...
| 发现证据 | `discovery_hits` | Run ID、实际Query、命中视频/频道、单次Query内部排名、发现时间 | 视频级原始发现证据；旧记录内容保留， 仅补充派生历史关键词族关联 |
...
## 历史发现迁移
 ...
 以前的 `discovery_hits` 没有原关键词和真实搜索批次 ID，因此仍然不会根据时间间隔猜测旧批次。 升级时保留全部旧视频命中证据，只删除并重建  自动生成的 `legacy-history` 派生记录。系统依据 Query Pack 已知长尾词以及历史中实际出现过的基础 Query，恢复“关键词族”，并按“推断原关键词 × Creator”聚合为稳定的 `legacy-keyword-*` 历史 Run；这些记录的 `base_query_source=inferred`，Dashboard/XLSX 显示为【历史推断】。 以后正式搜索的 `base_query_source=exact`，显示为【精确记录】。
...
## Persistent Jobs & Manual Creator Availability（）
...
同一 Creator 存在多个历史累计 GMV 快照时，Headline/博主库/二次指标只使用**最新 `captured_at` 快照**；不同采集时间的累计值不得相加。 不再执行 GMV 汇率换算，也不根据 Creator 国家推断币种。
