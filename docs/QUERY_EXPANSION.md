...
Dashboard 允许对当前语言的每个 Pack 增加或删除长尾词，并逐 Pack 启用/停用。每个词条还有独立勾选框：勾选表示本次检索使用；取消勾选表示保留该词但本次不使用；点击 × 才会从当前语言词库删除。新增词默认勾选。浏览器会将编辑结果保存在本地；`config/query_packs.json` 保留出厂默认词库。
